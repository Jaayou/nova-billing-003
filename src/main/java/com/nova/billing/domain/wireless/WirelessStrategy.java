...
public class WirelessStrategy implements DomainBillingStrategy {
    ...
    public void execute(BillingContext context) {
        System.out.println("  [v0.03 Strategy] === 'Wireless' Domain Strategy Executing ===");

        CalculationPipeline pipeline = pipelineLocator.getPipeline(context);
        System.out.println("    [v0.03 Strategy] -> Found Pipeline: " + pipeline.getClass().getSimpleName());

        List<CalculationStep> steps = pipeline.getSteps();

        System.out.println("    [v0.03 Strategy] -> Executing " + steps.size() + " Steps in order...");
        for (CalculationStep step : steps) {
            step.execute(context);
        }
        System.out.println("    [v0.03 Strategy] === Pipeline Execution Complete ===");
    }
}
